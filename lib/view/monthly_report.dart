...
class MonthlyReportView extends StatefulWidget {
  ...
  @override
  _MonthlyReportViewState createState() =>
      _MonthlyReportViewState();
}
...
class _MonthlyReportViewState extends State<MonthlyReportView> {
  ...
  void initState() {
    ...
    serviceTypeList.add(new CommonDropDownModel(id: "0",name: "All", title: ""));
    serviceTypeList.add(new CommonDropDownModel(id: "1",name: "Police Case", title: ""));
    serviceTypeList.add(new CommonDropDownModel(id: "2",name: "Accident", title: ""));
    serviceTypeList.add(new CommonDropDownModel(id: "3",name: "Fuel", title: ""));
    serviceTypeList.add(new CommonDropDownModel(id: "4",name: "Service", title: ""));
  ...
  void _loadData(BuildContext context) async {
    ...
    if(_selectedDropItem.vehicleId == "0") {
      Provider.of<ServiceProvider>(context, listen: false)
          .getMonthlyReport(AppConstant.startDate, AppConstant.endDate,
          _selectedDropItem.vehicleId, _selectedDropItem.serviceType,
          value.id.toString())
    }else{
      Provider.of<ServiceProvider>(context, listen: false)
          .getMonthlyReport(AppConstant.startDate, AppConstant.endDate,
          ...
          builder: (context, services, child) {
            ...
            if(  _selectedDropItem.serviceType == ""){
              ...
              if (serviceTypeList.isNotEmpty){
                ...
                _selectedDropItem.serviceType = serviceTypeList[0].id.toString();
              ...
              children: [
                ...
                Card(
                  child: DropDownListItem(
                      ...
                      requestType: "reportVehicleList",selectedItem:_selectedDropItem.vehicleId),
                ),
                Card(
                  child: DropDownListItem(
                      textTitle: "Service Type:",
                      list: serviceTypeList,
                      requestType: "serviceType", selectedItem: _selectedDropItem.serviceType),
                ),
                      ...
                      children:[
                        ...
                        if (services.expenseReportModel.isNotEmpty) ...[
                          Padding(
                            ...
                            child: Text("Total cost: ${totalCost(services.expenseReportModel)} TK", style: TextStyle(fontSize: 15),),
                          ),
                        ] else ...[
                          Text(""),
                        ],
                        ...
                        Row(
                          children: [
                            Expanded(flex: 1,child: Text(" Type", style: TextStyle(fontSize: 12),)),
                            Container(width: 1, height: 40, color: Colors.grey),

                            Expanded(flex: 1,child: Text(" Date", style: TextStyle(fontSize: 12),)),
                            Container(width: 1, height: 40, color: Colors.grey),
                            Expanded(flex: 1,child: Text(" Servicing", style: TextStyle(fontSize: 12),)),
                            Container(width: 1, height: 40, color: Colors.grey),
                            Expanded(flex: 1,child: Text(" Cost", style: TextStyle(fontSize: 12),)),
                            Container(width: 1, height: 40, color: Colors.grey),
                            Expanded(flex: 1,child: Text(" Vehicle", style: TextStyle(fontSize: 12),)),
                          ],
                        ),
                        Container(width: double.infinity, height: 1, color: Colors.grey),
                        ...
                        itemBuilder: (context, index) {
                          return services.expenseReportModel.isNotEmpty ? Row(
                            children: [
                              Expanded(flex: 1,child: Text(services.expenseReportModel[index].type ?? "", style: TextStyle(fontSize: 12),)),
                              Container(width: 1, height: 40, color: Colors.grey),

                              Expanded(flex: 1,child: Text(convertDate2(services.expenseReportModel[index].date ?? ""), style: TextStyle(fontSize: 12),)),
                              Container(width: 1, height: 40, color: Colors.grey),
                              Expanded(flex: 1,child: Text(services.expenseReportModel[index].eventName ?? "", style: TextStyle(fontSize: 12),)),
                              Container(width: 1, height: 40, color: Colors.grey),
                              Expanded(flex: 1,child: Text(services.expenseReportModel[index].cost ?? "", style: TextStyle(fontSize: 12),)),
                              Container(width: 1, height: 40, color: Colors.grey),
                              Expanded(flex: 1,child: Text(services.expenseReportModel[index].brandName ?? "", style: TextStyle(fontSize: 12),)),
                            ],
