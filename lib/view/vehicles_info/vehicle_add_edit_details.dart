...
class _VehicleAddEditViewState extends State<VehicleAddEditView> {
  ...
  void initState() {
    ...
    vehicleStatus.add(new CommonDropDownModel(id: "1",name: "Active", title: ""));
    vehicleStatus.add(new CommonDropDownModel(id: "-1",name: "InActive", title: ""));
    if (widget.vcDataModel.id != null) {
      ...
      _selectedDropItem.vehicleColourId =
          widget.vcDataModel.colorId.toString();
      ...
      print("vehicleColourName 3 ${_selectedDropItem.vehicleColourId }  ${widget.vcDataModel.colorId.toString()}");
    }else{
...
print("_selectedDropItem    ${ _selectedDropItem.vehicleTypeId}   ${ _selectedDropItem.vehicleBrandId}    ${ _selectedDropItem.vehicleModelId}   ${ _selectedDropItem.vehicleColourId}");

    Provider.of<ServiceProvider>(context, listen: false)
        .getVehicleType();
    Provider.of<ServiceProvider>(context, listen: false).getVehicleGeneralInfo("1", "",   _selectedDropItem.vehicleTypeId, "${AppConstant.userId}");
        ...
        child: Consumer<ServiceProvider>(builder: (context, services, child) {
          ...
          if(_selectedDropItem.vehicleBrandId == ""){
            ...
            for (var i = 0; i < services.vehicleBrandNameList.length; i++) {
              print("vehicleBrandName 4  "+services.vehicleBrandNameList[i].id.toString());
            }
            ...
            if (widget.vcDataModel.id != null) {
              ...
              print("vehicleBrandName  5 "+_selectedDropItem.vehicleBrandId);
              if (!exists){
                _selectedDropItem.vehicleBrandId = services.vehicleBrandNameList[0].id.toString();
              }
            }
            ...
            if (services.vehicleBrandNameList.isNotEmpty) {
              ...
              print("vehicleBrandName  6 "+_selectedDropItem.vehicleBrandId);
            }
          }

          if(_selectedDropItem.vehicleModelId == ""){
            if (services.vehicleModelNameList.isNotEmpty){
              _selectedDropItem.vehicleModelId = services.vehicleModelNameList[0].id.toString();
            }
                ...
                if (widget.vcDataModel.id != null) {
                  _selectedDropItem.vehicleModelId =
                      widget.vcDataModel.modelId.toString();
                  if (services.vehicleModelNameList.isNotEmpty) {
                    bool exists = services.vehicleModelNameList.any((file) => file.id == _selectedDropItem.vehicleModelId);
                    if (!exists){
                      _selectedDropItem.vehicleModelId = services.vehicleModelNameList[0].id.toString();
                    }
                  }
          ...
          print("vehicleModelName  ${_selectedDropItem.vehicleModelId }  ${services.vehicleModelNameList.length}");

          if(_selectedDropItem.vehicleColourId == ""){
            if (services.vehicleColourNameList.isNotEmpty){
              _selectedDropItem.vehicleColourId = services.vehicleColourNameList[0].id.toString();
              print("vehicleColourName  ${_selectedDropItem.vehicleColourId }  ${services.vehicleColourNameList.length}");
            }
          }else{
            if (services.vehicleColourNameList.isNotEmpty) {
              print("vehicleColourName 1 ${_selectedDropItem.vehicleColourId }  ${services.vehicleColourNameList.length}");
              bool exists = services.vehicleColourNameList.any((file) => file.id == _selectedDropItem.vehicleColourId);
              if (!exists){
                _selectedDropItem.vehicleColourId = services.vehicleColourNameList[0].id.toString();
                print("vehicleColourName 2 ${_selectedDropItem.vehicleColourId }  ${services.vehicleColourNameList.length}");
              }
            }
          }

          print("_selectedDropItem after data load   ${ _selectedDropItem.vehicleTypeId}   ${ _selectedDropItem.vehicleBrandId}    ${ _selectedDropItem.vehicleModelId}   ${ _selectedDropItem.vehicleColourId}");
          ...
          return Column(
            ...
            children: [
              ...
              AllDropDownItem(
                ...
                requestType: "vehicleType",selectedItem: _selectedDropItem.vehicleTypeId, isRequired: true,),
               SizedBox(height: 15),
              Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Expanded(
                      ...
                      child: AllDropDownItem(
                        ...
                        requestType: "BrandName", selectedItem: _selectedDropItem.vehicleBrandId,isRequired: true,),
                    ),
                  ...
                  children: [
                    Expanded(
                        ...
                        child:  AllDropDownItem(
                          ...
                          requestType: "ModelName",selectedItem: _selectedDropItem.vehicleModelId,isRequired: true,)),
                    Expanded( flex: 2,child: MaterialButton(
                      height: 30.0,
                      minWidth: 30.0,
                      color: MyTheme.buttonColor,
              ...
              EditListItem(
                    text: 'Manufacturing Year', nameController: modelYear, isYear: true, hintText: "Click to select year",),
              SizedBox(height: 10),

              AllDropDownItem(
                textTitle: "Vehicle Colour",
                list: services.vehicleColourNameList,
                requestType: "VehicleColour",selectedItem: _selectedDropItem.vehicleColourId,),
              SizedBox(height: 10),
              EditListItem(
                  text: 'Number Plate', nameController: vehicleNumber, hintText: "Type number plate",),
              SizedBox(height: 10),
              EditListItem(
                text: 'CC', nameController: cC, hintText: "Type CC",),
              SizedBox(height: 10),
              EditListItem(
                text: 'Tier Number', nameController: tierNumber, hintText: "Type Tier Number",),
              SizedBox(height: 10),
              EditListItem(text: 'Chassis Number', nameController: chasisNumber, hintText: "Type chassis number"),
              SizedBox(height: 10),
              EditListItem(
                text: 'Engine Number',
                nameController: engineNumber,  hintText: "Type engine number"
              ),
              SizedBox(height: 10),
              ImageUploadViewItem(
                  text: 'Vehicle Image',
              ...
              Padding(
                ...
                child: longButtons( (widget.vcDataModel.id == null) ? "SAVE" : "UPDATE", doUpdate),
              ),
            ],
          );
        },
