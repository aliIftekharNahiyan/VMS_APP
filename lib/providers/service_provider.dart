...
class ServiceProvider with ChangeNotifier {
  ...
  Future<List<ServiceDataModel>> getServicingList(String vehicleId, String userId) async {
    final responseData = await http.get(
        Uri.parse(AppUrl.getServicingList.replaceAll("_vehicleId", vehicleId).replaceAll("_ownerId", userId)));
    var result;
    ...
    if (responseData.statusCode == 200) {
      ...
      List<ServiceDataModel>? serviceList = ServiceModel.fromJson(jsonDecode(responseData.body)).data;
  ...
  Future<void> fetchVehicleDetails(String userId, String? requestType ) async {
    ...
    if (response.statusCode == 200) {
      ...
      if(requestType  == "report"){
        vehicleShortList.add(new CommonDropDownModel(id: "0",name: "All", title: ""));
      }
      for (var _list in vehicleModelList) {
        CommonDropDownModel service = new CommonDropDownModel(
            id: _list.id.toString(), name: "${_list.brandName}-${_list.modelName}", title: _list.modelName);
...
// garage list
  ...
  Future<void> fetchGarageList(String userId) async {
    ...
    if (responseData.statusCode == 200) {
      ...
      Iterable l = json.decode(responseData.body)['data'];
      List<GarageModel> garageModel = List<GarageModel>.from(l.map((model)=> GarageModel.fromJson(model)));
      ...
      for (var _list in garageModel) {
        CommonDropDownModel service = new CommonDropDownModel(
            id: _list.id.toString(), name: _list.gargeName, title: _list.gargeNameLocation);
  ...
  Future<void> getServiceListDropDown(String userId) async {
    ...
    final responseData = await http
        .get(Uri.parse(AppUrl.getServiceListDropDown.replaceAll("_userId", userId)));
    var result;
    if (responseData.statusCode == 200) {

      Iterable l = json.decode(responseData.body)['data'];
      List<ServiceNameModel> serviceNameModel = List<ServiceNameModel>.from(l.map((model)=> ServiceNameModel.fromJson(model)));
      print("getServiceListDropDown  "+serviceNameModel.toString());

     serviceNameList.clear();
      for (var _list in serviceNameModel) {
        CommonDropDownModel service = new CommonDropDownModel(
            id: _list.id.toString(), name: _list.serviceName, title: _list.timeStamp);
  ...
  Future<void> fetchDriverList(String userId) async {
    ...
    if (responseData.statusCode == 200) {
      ...
      Iterable l = json.decode(responseData.body)['data'];
      List<DriverListModel> driverList = List<DriverListModel>.from(l.map((model)=> DriverListModel.fromJson(model)));
  ...
  Future<void> getVehicleEnergyType() async {
    ...
    if (responseData.statusCode == 200) {
      ...
      Iterable l = json.decode(responseData.body)['data'];
      List<VehicleEnergyTypeModel> energyType = List<VehicleEnergyTypeModel>.from(l.map((model)=> VehicleEnergyTypeModel.fromJson(model)));
      ...
      for (var _list in energyType) {
        CommonDropDownModel service = new CommonDropDownModel(
            id: _list.id.toString(), name: _list.energySourceType, title: _list.createdBy);
  ...
  Future<dynamic> serviceUpdate(
      ...
      String ExpenseSlip,
      String partsImg,
      List<RequestServiceModel> ServiceLists
      ) async {
  ...
  Future<void> getSearchDriverList(String mobileNo, String name) async {
    ...
    final response = await http.get(Uri.parse(AppUrl.searchDriver.replaceAll("_mobileno", mobileNo).replaceAll("_name", name)));
    print(AppUrl.searchDriver.replaceAll("_mobileno", mobileNo));
    if (response.statusCode == 200) {
      // final Map<String, dynamic> responseData = json.decode(response.body);
      Iterable l = json.decode(response.body)['data'];
      List<SearchDriverModel> driverList = List<SearchDriverModel>.from(l.map((model)=> SearchDriverModel.fromJson(model)));
  ...
  Future<void> getPoliceFreezingList(String _userId, String _ownerId) async {
    final responseData = await http
        .get(Uri.parse(AppUrl.getPoliceFreezingDocList.replaceAll("_userId", _userId).replaceAll("_ownerId", _ownerId)));
    print("policeFreezingListApi url "+AppUrl.getPoliceFreezingDocList.replaceAll("_userId", _userId).replaceAll("_ownerId", _ownerId));

    var result;
    if (responseData.statusCode == 200) {

      Iterable l = json.decode(responseData.body)['data'];
      List<PoliceFreezingDocModel> policeFreezing = List<PoliceFreezingDocModel>.from(l.map((model)=> PoliceFreezingDocModel.fromJson(model)));
      ...
      for (var _list in policeFreezing) {
        CommonDropDownModel policeFreeing = new CommonDropDownModel(
            id: _list.id.toString(), name: _list.policeFreezingDocName, title: _list.timeStamp);
  ...
  Future<void> sendDriverAllocateRequest(String driverId, String ownerId, String vechileId) async {

    final response = await http.get(Uri.parse(AppUrl.sendDriverAllocateRequest.replaceAll("_driverId", driverId).replaceAll("_ownerId", ownerId).replaceAll("_vechileId", vechileId)));
    print(AppUrl.sendDriverAllocateRequest.replaceAll("_driverId", driverId).replaceAll("_ownerId", ownerId).replaceAll("_vechileId", vechileId));
    if (response.statusCode == 200) {
      final Map<String, dynamic> responseData = json.decode(response.body);
  ...
  Future<void> confirmDriverAllocateRequest(String Id) async {
    ...
    final response = await http.get(Uri.parse(AppUrl.confirmDriverAllocateRequest.replaceAll("_id", Id)));
  ...
  Future<void> removeDriverFromAllocation(String _ownerId,String _driverId) async {

    final response = await http.get(Uri.parse(AppUrl.removeDriverFromAllocation.replaceAll("_ownerId", _ownerId).replaceAll("_driverId", _driverId)));
    print(AppUrl.removeDriverFromAllocation.replaceAll("_ownerId", _ownerId).replaceAll("_driverId", _driverId));
    if (response.statusCode == 200) {
      final Map<String, dynamic> responseData = json.decode(response.body);
      // SendRequestRes sendRequestRes = SendRequestRes.fromJson(responseData);
      print(responseData);
  ...
  Future<void> getVehicleGeneralInfo(String infoId, String parentId, String vehicleTypeForModelId, String ownerId) async {

    print( "ownerID   ${ownerId}  ${AppConstant.userId}");
    final response = await http.get(Uri.parse(AppUrl.vehicleGeneralInfo.replaceAll("_infoId", infoId).replaceAll("_parentId", parentId).replaceAll("_vehicleTypeForModelId", vehicleTypeForModelId).replaceAll("_ownerId", ownerId)));
    print(AppUrl.vehicleGeneralInfo.replaceAll("_infoId", infoId).replaceAll("_parentId", parentId).replaceAll("_vehicleTypeForModelId", vehicleTypeForModelId).replaceAll("_ownerId", ownerId));
    if (response.statusCode == 200) {
      // final Map<String, dynamic> responseData = json.decode(response.body);

      final Map<String, dynamic> responseData = json.decode(response.body);
      VehicleGeneralInfoModel vehicleGeneralInfoModel = VehicleGeneralInfoModel.fromJson(responseData);
      CommonDropDownModel service = new CommonDropDownModel(
          id: "-1", name: "Please select", title: "");
      if (infoId == "1") {
